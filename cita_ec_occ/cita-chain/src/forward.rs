...
pub struct Forward {
    ...
    block_encoder: Arc<BlockEncoder>,
}
...
impl Forward {
    ...
    pub fn dispatch_msg(&self, key: &str, msg_bytes: &[u8]) {
        ...
        match RoutingKey::from(key) {
            routing_key!(Jsonrpc >> Request) => {
                ...
                info!("reply request took {:?} ms",sw.elapsed_ms());
            }
            ...
            routing_key!(Synchronizer >> RawBytes) => {
                info!("Received Synchronizer >> RawBytes");
                let ec_req = msg.take_raw_bytes().unwrap();
                info!("ec_req: {:?}", ec_req);
                self.reply_ec_req(ec_req, origin);
            }
            ...
            routing_key!(Snapshot >> SnapshotReq) => {
                // info!("recieve: {}", "Snapshot >> SnapshotReq");
    ...
    fn reply_request(&self, mut req: request::Request, imsg: Vec<u8>) {
        ...
        match req.req.unwrap() {
            ...
            Request::block_by_hash(rpc) => {
                ...
                match serde_json::from_str::<BlockParamsByHash>(&rpc) {
                    Ok(param) => {
                        let hash = param.hash;
                        let include_txs = param.include_txs;
                        match self.block_encoder.block_by_hash(H256::from(hash.as_slice())) {
                            Some(block) => {
                                let rpc_block = RpcBlock::new(
                                    hash,
                                    include_txs,
            ...
            //     match self.chain.block(block_height.block_id.clone().into()) {
            //         Some(block) => {
            //             let rpc_block = RpcBlock::new(
            ...
            //         }
            //         None => {
            //             // match self.block_encoder.block(block_height.block_id.clone().into()) {
            //             //     Some(_block) => {
            ...
            Request::block_by_height(block_height) => {
                match serde_json::from_str::<BlockNumber>(&block_height) {
                    Ok(block_height) => {
                        match self.block_encoder.block(block_height.clone().into()) {
                            Some(block) => {
                                // info!("block = {:?}", block);
                                // 序列化
                                ...
                                let serializedblock = SerializedBlock::new(0, bufblock);
                                let _ = serde_json::to_string(&serializedblock)
                                    .map(|data| response.set_block(data))
                                    .map_err(|err| {
                                        response.set_code(ErrorCode::query_error());
                                        response.set_error_msg(format!("{:?}", err));
            ...
            Request::transaction(hash) => {
                match self.block_encoder.full_transaction(H256::from_slice(&hash)) {
                    Some(ts) => {
                        response.set_ts(ts);
                        info!("block_encoder: {:?}", response);
                    }
            ...
            Request::transaction_proof(hash) => {
                match self.block_encoder.get_transaction_proof(H256::from_slice(&hash)) {
                    Some(proof) => {
            ...
            Request::transaction_receipt(hash) => {
                ...
                match self.block_encoder.localized_receipt(tx_hash) {
                    Some(receipt) => {
                    ...
                    None => {
                        match self.chain.localized_receipt(tx_hash) {
                            Some(receipt) => {
                                let rpc_receipt: RpcReceipt = receipt.into();
                                let serialized = serde_json::to_string(&rpc_receipt).unwrap();
                                response.set_receipt(serialized);
            ...
            Request::filter(encoded) => {
                ...
                if let Ok(rpc_filter) = serde_json::from_str::<RpcFilter>(&encoded).map_err(|err| {
                    ...
                    // let logs = self.block_encoder.get_logs(&filter);
            ...
            Request::block_header_height(block_height) => {
                let block_height: RpcBlockNumber =
                    serde_json::from_str(&block_height).expect("Invalid param");
                match self.block_encoder.get_block_header_bytes(block_height.clone().into()) {
                    Some(block_header_bytes) => {
                        response.set_block_header(block_header_bytes);
                    }
                    None => {
                        match self.chain.get_block_header_bytes(block_height.clone().into()) {
                            Some(block_header_bytes) => {
                                response.set_block_header(block_header_bytes);
                            }
                            None => {
    ...
    fn reply_ec_req(&self, ec_req: Vec<u8>, origin: u32) {
        let netmsg:EcNetMessage = serde_json::from_slice(ec_req.as_slice()).unwrap();
        info!("netmsg = {:?}", netmsg);
        let address = netmsg.node_address;
        if address == self.block_encoder.address {
            let req = netmsg.msg;
            let str_req = String::from_utf8(req.clone()).expect("Found invalid UTF-8");
            info!("str_req = {:?}", str_req);
            let datablock:Vec<u8>;
            match str_req.parse::<u64>() {
                Ok(number) => {
                    info!("number: {:?}", number);
                    datablock = match self.block_encoder.block_by_height_for_ec(number) {
                        Some(block) => {
                            block
                        },
                        None => {
                ...
                Err(_err) => {
                    datablock = match self.block_encoder.get_datablockmap(str_req) {
                        Some(block) => {
                            block
                        },
                        None => {
            ...
            // let datablock = self.block_encoder.get_datablockmap(str_ec_req).unwrap();

            let msg = Message::init(OperateType::Single, origin, datablock.into());
                trace!(
    ...
    // Check block group from remote and enqueue takes
    // 从远程检查块组并入队
    ...
    fn deal_snapshot_req(&self, snapshot_req: &SnapshotReq) {
        info!("snapshot_req:{:?}", snapshot_req);
...
// 本周任务：实现扩容缩容
// 进度：未完成
// 本周工作：看了节点加入后在程序中的处理流程，普通节点加入的处理方法已经和承宇学长讨论好并且实现出来了，接下来讨论实现共识节点加入处理方法，主要代码是在原来单节点方案上做了修改，比较分散，未统计