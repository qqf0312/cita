...
pub struct TxProof {
    ...
    pub receipt: Receipt,
    pub receipt_proof: cita_merklehash::Proof,
    pub block_header: Header,
    pub next_proposal_header: Header,
    pub proposal_proof: ProtoProof,
}
...
pub enum BlockInQueue {
    Proposal(OpenBlock), //提议
    ConsensusBlock(OpenBlock, ProtoProof), // 共识区块
    SyncBlock((OpenBlock, Option<ProtoProof>)), // 同步区块
}
...
pub struct Chain {
    ...
    // 生成区块的共识节点
    pub block_concensus_nodes: RwLock<HashMap<BlockNumber, ConsensusNodes>>,
    ...
    pub nodes: RwLock<Vec<Address>>,  //共识节点
    pub validators: RwLock<Vec<Address>>,  //验证者
...
impl Chain {
    pub fn init_chain(db: Arc<KeyValueDB>, 
        ...
        if let Some(nodes) = get_allnodes(&*db) {
        ...
        // let nodes: Vec<Address> = Vec::new();
        // let mut anodes :Vec<Address> = Vec::new();
        // anodes.clone_from(& allnodes.read().nodes);
        // let anodes = &allnodes.read().nodes.clone();
        // let mut newnodes =  anodes.clone();
        // for node in nodes {
        //     for node1 in anodes {
        //         if node != *node1 {
        //             newnodes.push(node);
        ...
        //     }
        // }
        // allnodes.write().nodes.clone_from(&newnodes);
        ...
        let chain = Chain {
            ...
            block_concensus_nodes: RwLock::new(HashMap::new()),
    ...
    fn set_config(&self, ret: &ExecutedResult) {
        ...
        let nodes: Vec<Address> = nodes  // 共识节点
            .iter()
            .map(|vecaddr| Address::from_slice(&vecaddr[..]))
            .collect();
        let validators = conf.get_validators();
        let validators: Vec<Address> = validators  // 验证者
            .iter()
            .map(|vecaddr| Address::from_slice(&vecaddr[..]))
            .collect();
        ...
        debug!(
        // info!(
            "consensus nodes {:?}, block_interval {:?}, validators {:?}, version {}",
            nodes, block_interval, validators, version
        );
    ...
    pub fn set_db_result(&self, ret: &ExecutedResult, block: &OpenBlock) {
        ...
        let conf = ret.get_config();
        let confnodes = conf.get_nodes();
        let mut nodes: Vec<Address> = confnodes  // 共识节点
            .iter()
            .map(|vecaddr| Address::from_slice(&vecaddr[..]))
            .collect();

        if self.get_nodes().nodes.len() < nodes.len() {
            self.set_allnodes(nodes.clone());
        }

        // 存储 对ec区间最后一个区块进行共识的节点Address
        if ret.get_executed_info().get_header().get_height() % self.sectionsize == self.sectionsize - 1 {
            nodes.sort();
            info!("nodes: {:?}", nodes);
            self.block_concensus_nodes.write().insert(ret.get_executed_info().get_header().get_height(), ConsensusNodes::new(nodes.clone()));
        }
        ...
        if !info.get_receipts().is_empty() {
            ...
            batch.write_with_cache( //db.rs L107
                db::COL_EXTRA,
        ...
        if !nodes.is_empty() {
            let mut write_consensus_nodes = self.block_concensus_nodes.write();  //写缓冲区
            batch.write_with_cache( //db.rs L107
                db::COL_EXTRA,
                &mut *write_consensus_nodes,  //写缓冲
                number,
                ConsensusNodes::new(nodes),  //写的内容
                CacheUpdatePolicy::Overwrite,
            );
            // 标记被使用了
            self.cache_man
    ...
    pub fn set_executed_result(&self, ret: &ExecutedResult, ctx_pub: &Sender<(String, Vec<u8>)>) {
        ...
        match block_in_queue {
            Some(BlockInQueue::ConsensusBlock(block, _)) => {
                // info!("Block in queue ConsensusBlock :{:#?}", block);
                // info!("Block in queue ConsensusBlock Height = {:#?}", block.number());
            ...
            Some(BlockInQueue::SyncBlock((block, op))) => {
                // info!("Block in queue SyncBlock :{:#?}", block);
                // info!("Block in queue SyncBlock Height = {:#?}", block.number());
                if op.is_some() {
                ...
                // info!("self.get_current_height() = {:?}", self.get_current_height());
                if number == self.get_current_height() + 1 {
                    // info!("number == self.get_current_height() + 1: {:?}", number);
                    if self.validate_hash(block.parent_hash()) {
                        // info!("self.validate_hash(block.parent_hash()):{:?}", block.parent_hash());
                        self.set_db_result(&ret, &block);
                        let tx_hashes = block.body().transaction_hashes();
                        self.delivery_block_tx_hashes(number, &tx_hashes, &ctx_pub);
                        self.broadcast_current_status(&ctx_pub);
        ...
        // 删掉block_map中小于当前节点区块高度的区块，以节省空间
        let mut guard = self.block_map.write();
        let new_map = guard.split_off(&self.get_current_height()); // 将BTreeMap分成两部分， new_map保存数值较大的一部分
        *guard = new_map;
    }
    ...
    pub fn set_block_consensus_nodes(&self, block_concensus_nodes:HashMap<BlockNumber, ConsensusNodes>) {
        for (key, value) in block_concensus_nodes {
            self.block_concensus_nodes.write().insert(key, value);
        }
    }

    pub fn get_block_consensus_nodes(&self, number: BlockNumber) -> Option<ConsensusNodes> {
        let result = self
        .db
        .read_with_cache(db::COL_EXTRA, &self.block_concensus_nodes, &number);
        self.cache_man
        .lock()
        .note_used(CacheId::BlockHeaders(number));
        result
    }

    pub fn remove_block_consensus_nodes(&self, block_number: BlockNumber) ->Option<ConsensusNodes> {
        self.block_concensus_nodes.write().remove(&block_number)
    }
    ...
    pub fn get_nodes(&self) -> ConsensusNodes {
    ...
    pub fn set_allnodes(&self, nodes: Vec<Address>) {
        ...
        if allnodes.len() != newnodes.len() {
            ...
            batch.write(db::COL_EXTRA, &AllNodes, &nodes);
            self.db.write(batch).expect("DB write failed.");
    ...
    pub fn clear_block_map(&self) {
        ...
        block_map
            ...
            .filter_map(|(key, value)| match value {
                BlockInQueue::SyncBlock(_) => None, //共识区块
                _ => Some((key, value)), //其他区块
            })
            .for_each(|(key, value)| {
