    ...
    */
            ...
            | routing_key!(Chain >> LocalSync) => {
                ...
                if rk == routing_key!(Consensus >> BlockWithProof){
                    ...
                    if btf_p.height == MAX {
                        ...
                        //将预执行结果更新至log中 2021.5.28
                        let signed_key = "consensus.signed_proposal";
                        self.update_backlog(signed_key, msg.clone());
                        self.grow_up();
                        info!("grow-up!(预执行)");
                        self.pre_execute_next_block();
                        info!("execute-next-block(prime)(预执行)");
                        //2021.5.28
                        switch = true;
                    }
                    //正式执行bwf
                    ...
                    info!("grow-up!");
                    if self.is_prime == true{
                        //若是主节点，则不用传输tdg图信息，因为在预执行时已经在节点信息中储存了
                        self.execute_next_block();
                        info!("execute-next-block(prime)");
                    }    
                    else{
    ...
    fn update_backlog(&mut self, key: &str, mut msg: Message) -> bool {
        match RoutingKey::from(key) {
            ...
            routing_key!(Consensus >> SignedProposal) => {
                //分两种情况 1.收到了主节点发来的block消息 2.正常的signedproposal /2021.5.28
                //let mut proposal = msg.take_signed_proposal().unwrap().clone();
                //qqf-code
                //2021.5.28
                if msg.take_signed_proposal().clone() == None{
                    let mut bwf = msg.take_block_with_proof().unwrap();
    ...
    //qqf-code
    fn pre_execute_next_block(&mut self) {
        let next_height = self.get_current_height() + 1;
        match self.backlogs.ready(next_height) {
            Ok(open_block) => {
                info!("postman send {}-th block to pre-executor", next_height);
                //self.fsm_req_sender.send(open_block.clone());
                self.fake_fsm_req_sender.send(open_block.clone());
            }
            Err(reason) => info!("{}", reason),
