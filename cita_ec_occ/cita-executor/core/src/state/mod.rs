...
impl AccountEntry {
    ...
    pub fn change_nonce(&mut self, _u :U256){
        //2021.5.28修改成nonce全部为0
        let mut a = self.account().unwrap().clone();
        let x : U256 = U256::from(0);
        a.change_nonce(x);
        /*
        if a.nonce() < &u {
            a.change_nonce(u) ;
    ...
    pub fn storage_at_mut(&mut self, address: &Address, key: &H256) -> trie::Result<H256> {
...

        {
            ...
            */
            ...
            // check local cache first without updating
            //在写集合找
            let local_cache = self.clone_cache();
            let mut _local_account = None;
            if let Some(maybe_acc) = local_cache.get(address) {
                match maybe_acc.account {
                    Some(ref account) => {
                        if let Some(value) = account.cached_storage_at(key) {
