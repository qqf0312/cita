...
pub trait FSM {
    ...
    fn pre_into_fsm(&mut self, open_block: OpenBlock) -> (BatchingOcc, ClosedBlock);
    ...
    fn pre_fsm_execute_in_parallel_version_2(&self, open_block: OpenBlock, rw_real: &mut RwSet, bocc_real: &mut BatchingOcc) -> StatusOfFSM;
    fn pre_fsm_execute_in_parallel_version_3(&self, open_block: OpenBlock, rw_real: &mut RwSet, bocc_real: &mut BatchingOcc) -> StatusOfFSM;
    ...
    fn test_2_deocc_fsm_execute(&mut self, executed_block: ExecutedBlock, open_block: OpenBlock, index: usize) -> ClosedBlock;
    //qqf-code-end
...
impl FSM for Executor {
    ...
    */
    fn test_deocc_fsm_execute(&self, open_block: OpenBlock, _index: usize){
        ...
        //打印temp_storage_cache_add_address
    ...
    fn test_1_deocc_fsm_execute(&mut self, executed_block: ExecutedBlock, open_block: OpenBlock, _index: usize) -> ClosedBlock{
        ...
        */
        ...
        for index in usize_iter{
            ...
            for x in iter{
                let (address,mut account_entry) = x.clone();
                if real_cache.contains_key(&address) == true{
                    ...
                    account_entry.change_nonce(nonce);
        ...
        info!("the cache after exchange:{:?}",&_executed_block.state.cache());
    ...
    //test_2 优化 6.2
    fn test_2_deocc_fsm_execute(&mut self, executed_block: ExecutedBlock, open_block: OpenBlock, _index: usize) -> ClosedBlock{
        //把tdg图给拆分成若干子图
        info!("----并发执行----");
        let g = self.tdg.clone();
        //制作每一笔交易的读写集!!!
        ...
        //执行块池 换成最大线程数量 6.2
        let mut executed_block_vec = Vec::new();
        let _len = total_vec_usize.clone().into_iter();
        ...
        loop {
            ...
            status = match status {
                StatusOfFSM::Initialize(open_block) => {
                    ...
                    //version3改良版
                    //self.pre_fsm_execute_in_parallel_version_2(open_block.clone(),&mut _rw_1,&mut _bocc_1);
                    //version2改良版
                    self.pre_fsm_execute_in_parallel_version_3(open_block.clone(),&mut rw,&mut bocc)
                ...
                pool.execute(move || {
                    ...
                    for n in iter{
                        ...
                        if !v.account.is_none(){
                            ...
                            let read = account.storage_cache_add_address();
    ...
    fn test_pre_fsm_execute_in_parallel(&self, open_block: OpenBlock, rw_real: &mut RwSet ,bocc_real: &mut BatchingOcc) -> StatusOfFSM {
        ...
        loop{
            ...
            for t in iter{
                ...
                info!("开始执行批处理交易");
                pool.execute(move || {
                    //_cache为上批交易处理后的结果（读写集）
                    //唯一一条数据key为0,而value就是上次执行的结果
    ...
    fn pre_fsm_execute_in_parallel_version_2(&self, open_block: OpenBlock, rw_real: &mut RwSet ,bocc_real: &mut BatchingOcc) -> StatusOfFSM {
        ...
        let n_work = 4;
        let pool = ThreadPool::new(n_work);
        //设置一个cache记录交易写过的写集(可以直接替换原有cache)
        //也是插入下一批论次的图
        let mut cache = HashMap::new();
        //loop直到map为空
        loop{
            ...
            for t in iter{
                ...
                pool.execute(move || {
                    ...
                    let self_cache = e.state.clone_cache();
                    //把inex和self_cache传入channel
                    tx.send((index.clone(), self_cache.clone())).unwrap();
                });
                //handles.push(handle);
            ...
            //info!("index_cache:{:?}",&cache_channel);
            //执行结束时间戳
            let executed_end_stamp = Local::now();
            info!("occ该批交易耗费时间：{:?}ms",executed_end_stamp.timestamp_millis() - executed_start_stamp.timestamp_millis());
            //获取读写集
            ...
            for index in iter{
                ...
                info!("{:?} in tdg",tran_info.tran_id);
                //把该交易的写集写入cache
                let key_set_iter = tran_info.write_set.unwrap_or_default().clone();
                let write_set_iter = key_set_iter.clone().into_iter();
                let value_set = tran_info.write_values.unwrap_or_default().clone();
                let mut key_index = 0;
                for key in write_set_iter{
                    let key_string = key.clone().split_off(66);
                    let value_string = value_set[key_index.clone()].clone().split_off(66);
                ...
                //将原有cache替换成
                let mut real_cache = executed_block.state.clone_cache();
                //info!("version2预执行“之前”real_cache:{:?}",&real_cache);
                let hash_map = cache_channel.get(&index).unwrap().clone();
                let iter = hash_map.clone().into_iter();
                for x in iter{
                    let (address, mut account_entry) = x.clone();
                    if real_cache.contains_key(&address) == true{
                        ...
                        let nonce = new_account.nonce().clone();
                        //如果noce大于old的，则取最大的
                        //2021.5.28 change_nonce吧nonce改成0
                        old_account_entry.change_nonce(nonce);
                        //let mut old_storage_changes = old_account.storage_changes().clone();
                        let new_storage_changes = new_account.storage_changes().clone();
                        let iter = new_storage_changes.into_iter();
                ...
                //info!("version2预执行“以后”real_cache:{:?}",&real_cache);
                executed_block.exchange_state_cache(real_cache);
            }
            info!("cache:{:?}",&cache);
            //康康生成的tdg
            //info!("康康生成的tdg");
        ...
        info!("version2: 预执行之后的cache:{:?}",executed_block.state.cache());
        info!("topo:{:?}",tdg_toposort);
        //提交预执行执行结果
        executed_block.state.commit().expect("failed to commit state trie");
        //运行完所有交易直接结束
        let _x = Arc::try_unwrap(only_executed_block).unwrap().into_inner().unwrap();
        let end_time = Local::now();
        info!("version2消耗时间（修改后）:{:?}ms",end_time.timestamp_millis() - start_time.timestamp_millis());
        StatusOfFSM::Finalize(executed_block)
    }
    //优化过后的预执行 2021.5.26
    fn pre_fsm_execute_in_parallel_version_3(&self, open_block: OpenBlock, rw_real: &mut RwSet ,bocc_real: &mut BatchingOcc) -> StatusOfFSM {
        info!("version 3 strat");
        let start_time = Local::now();
        //唯一制定运行完所有交易的块 executed_block
        let executed_block = self.to_executed_block(open_block.clone());
        let new_rw = RwSet::new();
        ...
        loop{
            ...
            //一整批occ时间戳
            let executed_start_stamp = Local::now();
            //给存放交易的transaction_map切分成线程数量的小块。
            let mut split_set_transactions = Vec::new();
            let to_split = transaction_map.clone();
            let transaction_number_of_a_split = to_split.len() / n_work;
            let iter = transaction_map.clone().into_iter();
            let mut temp = HashMap::new();
            for t in iter{
                let (index, transaction)= t;
                temp.insert(index.clone(), transaction.clone());
                if temp.len() >= transaction_number_of_a_split{
                    split_set_transactions.push(temp.clone());
                    temp.clear();
                }
            }
            split_set_transactions.push(temp.clone());
            //查看分组情况
            let iter = split_set_transactions.iter();
            for vec in iter{
                let iter = vec.iter();
            ...
            //给trans_map的Iter
            //let iter = transaction_map.clone().into_iter();
            let iter_vec = split_set_transactions.clone().into_iter();
            let _arc_cache = Arc::new(cache.clone());
            //处理transaction_map中每一笔交易
            for t_map in iter_vec{
                //处理单笔交易的executedblock:e
                let mut e = self.to_executed_block(open_block.clone());
                let conf = self.sys_config.block_sys_config.clone();
                //let mut transaction = executed_block.body().transactions[index - 1].clone();
                ...
                //let iter = t_map.clone().into_iter();
                //for t in iter
                let rw = Arc::clone(&rw);
                let mut temp_hash = HashMap::new();
                temp_hash.insert(0, cache.clone());
                ...
                pool.execute(move || {
                    ...
                    let iter = t_map.clone().into_iter();
                    for tran in iter{
                        let (index, mut transaction) = tran;
                        if economical_model == EconomicalModel::Charge {
                            transaction.gas_price = quota_price;
                        }
                        e.apply_transaction(&engine, &transaction, &conf);
        ...
        info!("version3: 预执行之后的cache:{:?}",executed_block.state.cache());
        info!("topo:{:?}",tdg_toposort);
        //提交预执行执行结果
        executed_block.state.commit().expect("failed to commit state trie");
        //运行完所有交易直接结束
        let _x = Arc::try_unwrap(only_executed_block).unwrap().into_inner().unwrap();
        let end_time = Local::now();
        info!("version3消耗时间（修改后）:{:?}ms",end_time.timestamp_millis() - start_time.timestamp_millis());
        StatusOfFSM::Finalize(executed_block)
    }
    //执行每一次交易都clear缓存的
