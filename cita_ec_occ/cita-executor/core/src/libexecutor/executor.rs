...
pub struct Executor {
    ...
    //pub cache: HashMap<String, AccountEntry>,
    //qqf-code-end
        ...
        loop {
            match self.recv() {
                ...
                (None, Some(block), None, None) => {
                    ...
                    info!("收到openblock");
                    //判断内部交易数量是否为1（部署合约使用）
                    let number = block.body().transactions.len();
                    info!("open block 交易的数量:{:?}",&number);
                    //倘若为1，直接进入串行执行
                    if number == 1{
                        info!("不需要预执行");
                        let fsm_resp = self.into_fsm(block);
                ...
                (None, None, Some(block), None) => {
                    ...
                    //判断内部交易数量是否为1（部署合约使用）
                    let number = block.body().transactions.len();
                    info!("fake block 交易的数量:{:?}",&number);
                    //获得预执行过后的函数依赖图tdg
                    let (bocc, closed_block) = self.pre_into_fsm(block.clone());
                    ...
                    //交易数量为1的时候，直接按照原来的cita执行
                    if number == 1{
                        info!("直接按照原来的cita执行");
                        let _closed_block = self.into_fsm(block.clone());
                        self.fsm_resp_sender.send(_closed_block);
                    }
                    else{
                        //在预执行结束后发送执行结果closeblock 2021.5.28
                        self.fsm_resp_sender.send(closed_block);
                    }
                    info!("已发送CLOSEDBLOCK(预执行)");
                    /*
                    //反序列化
                    //info!("开始反序列化");
