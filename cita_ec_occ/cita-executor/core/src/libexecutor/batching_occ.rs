    ...
    */
    ...
    pub fn get_tdg_transactioninfo(&self){
        ...
        for n in nodes_iter{
            ...
            let iter = node.read_set.clone().unwrap_or_default().into_iter();
            let mut index: usize = 0;
            info!("read读集:");
            for n_r in iter{
                let temp = node.read_values.clone().unwrap_or_default();
                info!("key:{} value:{:?}",n_r.clone(),temp[index].clone());
                index = index + 1;
            }
            let iter = node.write_set.clone().unwrap_or_default().into_iter();
            let mut index: usize = 0;
            info!("write写集:");
            for n_w in iter{
                let temp = node.write_values.clone().unwrap_or_default();
