...
impl LocalMessage {
    ...
    pub fn handle(self, service: &mut Network) {
        ...
        match rt_key {
            ...
            routing_key!(Chain >> RawBytes) => {
                info!("Received Chain >> RawBytes");
                let msg = ProtoMessage::try_from(&self.data).unwrap();
                let dst = msg.get_origin() as usize;
                info!("Chain >> RawBytes dst = {:?}", dst);
                send_message(
                    &service.nodes_mgr_client,
            ...
            routing_key!(Auth >> RawBytes) => {
                info!("Received Auth >> RawBytes");
                let msg = ProtoMessage::try_from(&self.data).unwrap();
                let dst = msg.get_origin() as usize;
                info!("Auth >> RawBytes dst = {:?}", dst);
                send_message(
                    &service.nodes_mgr_client,
            ...
            _ => {
                error!("[Network] Unexpected key {} , {:?}, from Local", self.key, self.data);
            }
        }
    }
    ...
    pub fn handle(self, service: &mut Network) {
        ...
        match rt_key {
            ...
            routing_key!(Chain >> RawBytes) => {
                info!("Received Chain >> RawBytes");
                service.mq_client.pub_sync_request(PubMessage::new(
                    routing_key!(Synchronizer >> RawBytes).into(),
                    self.data,
                ));
            }
